...
(function(app, request, Promise) {
    app.providers = {
        ...
        ,google: {
            ...
            ,url: function(address, city) {
                return this.apiHost + this.apiPath + "?sensor=false&address=" + encodeURIComponent(address + ", " + city);
            }
    ...
    app.geocode = function(address, provider, city) {
        var promise = new Promise();
        if(app.providers[provider] === undefined) {
            promise.reject("Invalid provider");
        }
        var url = app.providers[provider].url(address, city);
        request(url, function(error, response, body) {
            if(error || response.statusCode !== 200) return promise.reject();
            
            promise.resolve(app.providers[provider].parse(JSON.parse(body)));
        });
        return promise;
    };
})(module.exports, request, Promise);