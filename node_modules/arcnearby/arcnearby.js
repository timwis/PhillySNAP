...
(function(app, _, request, Promise) {
    
    /**
     * Put this here so it's public
     */
     ...
     */
    app.getNearby = function(service, coords, radius, overrideParams) {
        var promise = new Promise()
            ,url = app.config.apiHost + app.config.apiPath + (app.config.services[service] || "")
            ,params = ( ! _.isEmpty(overrideParams)) ? _.defaults(overrideParams, app.config.params) : _.clone(app.config.params);
        ...
        request({url: url, qs: params}, function(error, response, body) { // Query REST service
            if(error) return promise.reject(response, body);
            
            var data = JSON.parse(body);
            promise.resolve((data.features !== undefined) ? app.sortByDistance(data.features, coords) : {}); // Sort by distance
        });
        return promise;
    };

})(module.exports, _, request, Promise);